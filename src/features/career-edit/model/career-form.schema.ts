...
export const careerFormSchema = z
  .object({
    name: z
      ...
      .regex(
        /^[가-힣a-zA-Z]+(\.[가-힣a-zA-Z]+)?$/,
        '이름에 특수문자나 숫자를 사용할 수 없습니다. 점(.)은 한 번만 사용할 수 있습니다.'
      ),
    company: z
      .string()
      .min(1, '회사명을 입력해주세요.')
      ...
      .regex(
        /^[가-힣a-zA-Z0-9\s()&.]+$/,
        '회사명은 한글, 영문, 숫자와 특수문자(점, 괄호, &) 및 공백만 사용 가능합니다.'
      ),
    department: z
      .string()
    ...
    lined_number: z
      ...
      .regex(
        /^(?:\d{2,3}-\d{3,4}-\d{4})?$/,
        '올바른 전화번호 형식이어야 합니다. (예: 02-123-4567)'
      )
      .optional()
      .or(z.literal('')),
